...
X = pd.read_table('Datasets/breast-cancer-wisconsin.data', sep=',', 
...
scaler = preprocessing.MaxAbsScaler().fit(X=X_train) ## 0.95714
##scaler = preprocessing.MinMaxScaler().fit(X=X_train) ## 0.96571
##scaler = preprocessing.StandardError().fit(X=X_train) ## 0.83425
##scaler = preprocessing.Normalizer().fit(X=X_train ## 0.83428
##scaler = preprocessing.RobustScaler().fit(X=X_train) ## 0.95714
...
#
# .. your code here ..
knmodel = KNeighborsClassifier(n_neighbors=5).fit(X_train, y_train)
